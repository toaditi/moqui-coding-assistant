#!/usr/bin/env python3
"""Validate the self-contained Codex plugin package in this repository."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Repository root containing .codex-plugin/ (default: this repository).",
    )
...
def validate(root: Path) -> list[str]:
    ...
    if not manifest_path.is_file():
        return [f"missing Codex plugin manifest: {manifest_path}"]

    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"cannot read Codex plugin manifest {manifest_path}: {exc}"]
    ...
    for manifest_field in ("hooks", "mcpServers"):
        configured_path = manifest.get(manifest_field)
        if configured_path and not (root / configured_path).is_file():
            errors.append(f"manifest {manifest_field} path does not exist: {configured_path}")
...
def main() -> int:
    ...
    if errors:
        ...
        print(f"Codex plugin validation failed with {len(errors)} error(s).", file=sys.stderr)
        return 1
